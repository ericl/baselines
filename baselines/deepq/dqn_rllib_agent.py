...
DEFAULT_CONFIG = dict(
    ...
    # Length of annealing schedule.
    schedule_max_timesteps=100000,
    # What fraction of annealing schedule to explore for.
    exploration_fraction=0.1,
    # Final value of exploration schedule.
    ...
    def _init(self):
        if self.config["apex"]:
            self.optimizer = ApexOptimizer.make(
                BaselinesDQNEvaluator, [self.config, self.env_creator],
            ...
            self.optimizer = LocalSyncReplayOptimizer.make(
                BaselinesDQNEvaluator, [self.config, self.env_creator], 0, {
                    k: self.config[k]
                    for k in [
                        "learning_starts", "buffer_size", "sample_batch_size",
                        "prioritized_replay", "prioritized_replay_alpha",
